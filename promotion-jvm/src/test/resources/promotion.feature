...
| code                      | script                                      |
| x_off_for_y_spent         | return ${origin} >= ${threshold}? ${off}: 0 |
| x_off_for_every_y_spent   | return ${origin} / ${threshold} * ${off}    |
| x_off_for_y_ordered       | def quantities = ${quantities}; def prices = ${prices}; def temp = 0; for (int i=0;i<quantities.size;i++) {if (${threshold} <= quantities[i]) temp += prices[i]*${off};}; return temp |
| 2_stage_x_off_for_y_spent | if (${origin} >= ${threshold2}) return ${off2}; if (${origin} >= ${threshold1}) return ${off1}; return 0;|
